...
describe('lib/Eth', () => {
  ...
  describe('getBalance', () => {
    ...
    it('passes in the address & blockNumber', () => {
      return instance.getBalance(address, 0x123).then(() => {
        expect(scope.body.eth_getBalance.params).to.deep.equal([address.toLowerCase(), '0x123']);
      });
    });

    it('returns the balance', () => {
      return instance.getBalance(address, 0x123).then((result) => {
        expect(result.toString(16)).to.deep.equal('123');
      });
    });
  });
});
