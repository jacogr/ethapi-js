import { outAddress, outNumber } from '../util/format';
import { inAddress, inBlockNumber, inHex, inNumber } from '../util/format'; // eslint-disable-line no-duplicate-imports
...
export default class Eth {
  ...
  compileLLL (code) {
    return this._transport
      .execute('eth_compileLLL', inHex(code));
  }

  compileSerpent (code) {
    return this._transport
      .execute('eth_compileSerpent', inHex(code));
  }

  compileSolidity (code) {
    return this._transport
      .execute('eth_compileSolidity', inHex(code));
  }
  ...
  sendRawTransaction (data) {
    return this._transport
      .execute('eth_sendRawTransaction', inHex(data));
  }
