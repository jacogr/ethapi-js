...
import { outAddress, outNumber } from './format';
import { inAddress, inBlockNumber, inData, inHex, inNumber } from './format'; // eslint-disable-line no-duplicate-imports
...
describe('util/format', () => {
  ...
  describe('inNumber()', () => {
    it('formats existing BigNumber into hex', () => {
      expect(inNumber(new BigNumber(0x123456))).to.equal('0x123456');
    });

    it('formats hex strings into hex', () => {
      expect(inNumber('0x123456')).to.equal('0x123456');
    });
