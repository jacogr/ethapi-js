...
describe('transport/JsonRPC', () => {
  describe('container', () => {
    it('encodes the body correctly', () => {
      const bdy = transport.encodeBody('someMethod', ['param1', 'param2']);
    ...
    it('encodes the options correctly', () => {
      const opt = transport.encodeOptions('someMethod', ['param1', 'param2']);
      const enc = {
        method: 'POST',
        headers: {
        ...
        body: `{"jsonrpc":"2.0","method":"someMethod","params":["param1","param2"],"id":${transport._id - 1}}`
      };
    ...
    it('properly constructs the endpoint', () => {
      expect(transport.getEndpoint()).to.equal(`http://${TEST_HOST}:${TEST_PORT}/`);
    });
  });
