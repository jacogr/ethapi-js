...
const transport = new Http(TEST_HOST, TEST_PORT);
...
describe('transport/Http', () => {
  describe('container', () => {
    ...
    it('properly constructs the endpoint (path)', () => {
      expect(new Http('/rpc')._getEndpoint()).to.equal('/rpc');
    });

    it('properly constructs the endpoint (host & port)', () => {
  ...
  describe('logging', () => {
    ...
    it('logs the HTTP error', () => {
      const scope = mockHttp([{ method: 'eth_call', reply: {}, code: 500 }]);
    ...
    it('logs the RPC error', () => {
      const scope = mockHttp([{ method: 'eth_call', reply: { error: 'test error' } }]);
    ...
    it('logs the RPC result', () => {
      const scope = mockHttp([{ method: 'eth_call', reply: { result: '0x123456789' } }]);
