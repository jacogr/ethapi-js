...
describe('contract/Contract', () => {
  ...
  describe('_encodeOptions', () => {
    ...
    it('encodes the at address in to', () => {
      expect(
        contract._encodeOptions(func, {}, VALUES).to
      ).to.equal(ADDR);
    });

    it('allows the override to as specified', () => {
      expect(
        contract._encodeOptions(func, { to: '0xabc' }, VALUES).to
      ).to.equal('0xabc');
    });

    it('encodes the parameters into data', () => {
      expect(
        contract._encodeOptions(func, {}, VALUES
      ).data).to.equal(ENCODED);
    });

    it('maps the original options into the result', () => {
      expect(
        contract._encodeOptions(func, { original: 'test' }, VALUES
      ).original).to.equal('test');
    });
  });
  ...
  describe('sendTransaction', () => {
    ...
    it('encodes options and mades an eth_sendTransaction call', () => {
      return contract
        .sendTransaction(func, { someExtras: 'foo' }, VALUES)
        .then(() => {
